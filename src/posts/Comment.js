...
class Comment extends Component {
    ...
    isValid = () => {
        ...
        if(!text.length > 0 || text.length> 150) {
            this.setState({error: "Comment should not be empty and should be less than 150 characters long"})
            return false
        }
        return true
    }
           ...
           <div>
               ...
               <form onSubmit = {this.addComment}>
                   <div className="form-group">
                        <input type="text" onChange={this.handleChange} value={this.state.text} className="form-control" placeholder="whats your say..."/>
                            ...
                            <div key={i}>
                                    <div>
                                        <Link to={`/user/${comment.postedBy._id}`}>
                                            <img style={{borderRadius:"50%",border: "1px solid black"}} className = "float-left mr-2" height="30px" width="30px"  src={`${process.env.REACT_APP_API_URL}/user/photo/${comment.postedBy._id}?${new Date().getTime()}`} onError={i => (i.target.src = `${defaultprofile}`)} alt={comment.postedBy.name} />
                                        </Link>
                                            <div>
